...
def _make_invalid_datasource(provider: str) -> str:
    invalid_path = r"C:\__qrator_missing__\missing.xxx" if os.name == "nt" else "/__qrator_missing__/missing.xxx"
    p = (provider or "").lower()
    if p == "delimitedtext":
...
def _write_qgz_with_aux(xml_root: etree._Element, output_path: str, meta: Dict, kept_layer_ids: Set[str]) -> bool:
    """
    Écrit xml_root dans un .qgz (output_path).
    ...
    Si la source des fichiers auxiliaires (meta["source_path"]) est le même fichier que output_path
    (cas d'une fusion "en place", où l'on réécrit le projet existant), on écrit d'abord dans un fichier
    temporaire puis on le déplace vers output_path à la fin, pour ne pas écraser la source avant de
    l'avoir lue.
    """
    try:
        # DEBUG : écrire le XML filtré/fusionné à côté
        try:
        ...
        same_as_source = bool(src_path) and os.path.exists(src_path) and \
            os.path.abspath(src_path) == os.path.abspath(output_path)
        write_target = output_path
        tmp_output = None
        if same_as_source:
            fd, tmp_output = tempfile.mkstemp(suffix=".qgz")
            os.close(fd)
            write_target = tmp_output

        with zipfile.ZipFile(write_target, "w", zipfile.ZIP_DEFLATED) as zipf:
        ...
        os.unlink(tmp_qgs)
        print(f"[QRator] Saved project to {output_path}")
        return True
    except Exception as e:
...
def save_new_project(output_path: str, xml_root: etree._Element, selected: Dict, meta: Dict = None) -> bool:
    ...
    # Déconnecter les couches locales si demandé par l'UI
    ...
    return _write_qgz_with_aux(filtered, output_path, meta, kept_layer_ids)
...
def save_merged_project(existing_project_path: str,
    ...
    try:
        ...
        base_root, base_meta = open_project(existing_project_path)
        ...
        # 4) Déconnecter les couches locales si demandé par l'UI
        if selected.get("disconnect_local"):
            try:
        ...
        # 5) Écrire sur disque en réutilisant/élaguant le stockage auxiliaire du projet A
        kept_layer_ids = _collect_known_layer_ids(merged_root)
        return _write_qgz_with_aux(merged_root, output_path, base_meta, kept_layer_ids)
